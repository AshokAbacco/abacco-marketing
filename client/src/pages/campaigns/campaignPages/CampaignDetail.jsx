...
const Card = ({ children, className = "" }) => (
  <div
    className={`bg-white/80 backdrop-blur-sm border border-sky-200/50 rounded-2xl shadow-lg ${className}`}
  >
    {children}
  </div>
);
...
const CardHeader = ({ children }) => (
  <div className="px-6 py-5 border-b border-sky-100 bg-gradient-to-r from-sky-50 to-blue-50">
    {children}
  </div>
);
...
export default function CampaignDetail() {
  ...
  const createFollowUp = async () => {
    ...
    const hasBody =
      followUpBody && followUpBody.replace(/<[^>]*>/g, "").trim().length > 0;
    ...
    if (loadingRecipients) {
      setModal({
        ...
        type: "error",
        message:
          "Still loading this campaign's address list — please wait a moment.",
      });
      return;
    }
    ...
    if (followUpRecipientCount === 0) {
      setModal({
        ...
        type: "error",
        message:
          "This campaign has no successfully sent recipients to follow up with.",
      });
      return;
    }
    ...
    try {
      ...
      if (!sendData.success) {
        throw new Error(
          sendData.message || "Failed to send follow-up campaign",
        );
      }
                    ...
                    campaigns.map((c) => {
                      ...
                      return (
                        <option key={c.id} value={c.id}>
                          {c.name} — {ordinal} Follow-up ({completedCount}{" "}
                          recipients)
                        </option>
                      );
                    })}
                  ...
                  <div className="flex items-center justify-between gap-3 mt-3">
                    ...
                    {campaignsHasMore && (
                      <button
                        type="button"
                        onClick={() =>
                          fetchCampaigns(followupLevel, { append: true })
                        }
              ...
              <CardContent className="space-y-4">
                ...
                {(() => {
                  ...
                  const overLimit =
                    dailyLimit &&
                    recipientCount > 0 &&
                    recipientCount > dailyLimit.remaining;
                  return overLimit ? (
                    <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl p-4 mt-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-red-600 mt-0.5 leading-relaxed">
                          ...
                          {dailyLimit.dailyLimit.toLocaleString()}). This
                          follow-up needs{" "}
                          <strong>{recipientCount.toLocaleString()}</strong>{" "}
                          recipients. Please try again tomorrow.
                        </p>
                      </div>
                    </div>
                  ) : null;
                ...
                {loadingRecipients ? (
                  <CardContent>
                    <div className="flex flex-col items-center justify-center gap-3 py-16 text-sky-600">
                      ...
                      <p className="text-sm font-semibold">
                        Loading recipients &amp; previous message…
                      </p>
                    </div>
                  </CardContent>
                ) : recipientsError ? (
                  ...
                  <CardContent className="space-y-6 text-base text-black">
                    ...
                    <div className="border-2 border-sky-200 rounded-xl p-5 bg-white shadow-sm">
                      <div className="font-bold text-sky-800 mb-3 uppercase tracking-wide text-sm">
                        Follow-up
                      </div>
                      <div
                        dangerouslySetInnerHTML={{
                          __html: buildFollowupWithSignature(),
                        }}
                      />
                    </div>
                    ...
                    <div className="border-t-2 border-sky-200 pt-5 text-sm space-y-2 bg-gradient-to-br from-sky-50 to-blue-50 p-5 rounded-xl border-2">
                      ...
                      <div className="font-bold text-sky-900">
                        Sent:{" "}
                        <span className="font-normal text-slate-700">
                          {new Date(loadedCampaign.createdAt).toLocaleString()}
                        </span>
                      </div>
                      ...
                      <div className="text-xs text-sky-600 font-semibold">
                        Will send to total {loadedCampaign?.sentCount ?? 0}{" "}
                        recipients (distributed automatically)
                      </div>
            ...
            <Card className="sticky top-6">
              ...
              <CardContent className="space-y-5 text-sm">
                ...
                <div className="bg-gradient-to-br from-sky-50 to-blue-50 p-4 rounded-xl border border-sky-200">
                  <p className="text-xs text-sky-700 font-bold uppercase tracking-wide mb-1.5">
                    Campaign
                  </p>
                ...
                <div className="bg-gradient-to-br from-sky-50 to-blue-50 p-4 rounded-xl border border-sky-200">
                  ...
                  {!loadingRecipients &&
                    ...
                    loadedCampaign &&
                    allSentRecipients.length > 0 &&
                    allSentRecipients.length !==
                      (loadedCampaign.sentCount ?? 0) && (
                      <p className="text-xs text-slate-500 mt-1">
                        Showing {allSentRecipients.length} of{" "}
                        {loadedCampaign.sentCount} for preview — the full list
                        loads when you click Create Follow-up.
                      </p>
                    )}
